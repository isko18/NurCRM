...
- `company_id` (обязательный) - UUID компании
- `branch_id` (опциональный) - UUID филиала

**Пример подключения:**
```
ws://your-domain/ws/cafe/orders/?token=<JWT>&company_id=<uuid>&branch_id=<uuid>
```
...
**Пример подключения:**
```
ws://your-domain/ws/cafe/tables/?token=<JWT>&company_id=<uuid>&branch_id=<uuid>
```
...
const branchId = 'your-branch-uuid'; // опционально

const ws = new WebSocket(
  `ws://your-domain/ws/cafe/orders/?token=${token}&company_id=${companyId}&branch_id=${branchId}`
);
...
- `4003` - Пользователь не аутентифицирован
- `4004` - Не указан `company_id`
- `4005` - Пользователь не имеет доступа к указанной компании
