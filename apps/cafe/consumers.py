...
class CafeKitchenConsumer(AsyncWebsocketConsumer):
    """
    WebSocket consumer для кухни (повара): уведомления о задачах кухни.
    Company и branch определяются автоматически из JWT токена пользователя.
    Опционально можно указать branch_id в query для выбора конкретного филиала (для owner/admin).
    URL: ws/cafe/kitchen/?token=<JWT>&branch_id=<uuid> (опционально)
    """
    ...
    async def connect(self):
        ...
        # Формируем имя группы для подписки (кухня)
        if self.branch_id:
            self.group_name = f"cafe_kitchen_{self.company_id}_{self.branch_id}"
        else:
            self.group_name = f"cafe_kitchen_{self.company_id}"
    ...
    async def kitchen_task_ready(self, event):
        ...
        try:
            ...
            logger.error(f"[CafeKitchenConsumer] Error in kitchen_task_ready: {e}", exc_info=True)
