...
- `phone` — **обязательное** поле при создании контрагента (номер телефона контрагента).
...
### 4.2.2 Партнёрство компаний (склад + касса / инкассация)

Одна заявка на партнёрство открывает и **склады**, и **кассы** партнёра. Полное описание: **[docs/warehouse_stock_partnership_frontend.md](../../docs/warehouse_stock_partnership_frontend.md)** (товарный `transfer/`, каталог, инкассация `cash-incassations/`).
...
**Скидки (как считается `line_total` и `total`):**
- На строку действует **одна** скидка, скидки не суммируются:
  - `items[].discount_percent` — приоритетный процент по позиции;
  - если он `0`, вместо него подставляется `discount_percent` документа (общая скидка);
  - `items[].discount_amount` (фиксированная сумма по строке) применяется, **только** если процента нет ни в строке, ни в документе.
- Поэтому **нельзя** слать процент и его же сумму одновременно: для товара 100 сом с 5% нужно `discount_percent: "5.00"`, `discount_amount: "0.00"` — придёт `line_total = 95.00`. Если прислать ещё и `discount_amount: "5.00"`, сумма всё равно будет проигнорирована (раньше вычиталась второй раз и давала 90.00).
- `effective_discount_percent` в ответе — процент, который реально применился к строке.
- `discount_amount` **документа** — отдельная фиксированная скидка на весь документ, она вычитается из суммы строк: `total = Σ line_total − document.discount_amount`.
- `line_total` и `total` считает сервер; присланные клиентом значения игнорируются.
...
- `prepayment_amount` (опционально, только для `payment_kind="credit"`) — **предоплата по долгу**. Если передать сумму \(> 0\), при проведении документа сервер автоматически создаст и проведёт денежный документ (`MONEY_RECEIPT`/`MONEY_EXPENSE`) на сумму предоплаты (используя `cash_register` и `payment_category` из документа или автоматически выбранные, если в текущем company/branch ровно по одному варианту).
- Для остальных типов документов поле можно не передавать (или `null`).

**Касса (важно):**
- В `CASH_PENDING` попадают **только** документы с `payment_kind="cash"` (где нужно создавать денежное движение).
...
{ "allow_negative": true }
```
- при успехе: 200 + сериализованный документ (status станет `POSTED` для `credit` и `CASH_PENDING` для `cash`, `number` будет сгенерирован).
