...
```
- Ответ `201`.
- Скидки по строке не суммируются: действует `items[].discount_percent`, при нулевом — `discount_percent` документа, и только если процента нет нигде — `items[].discount_amount`. Процент и его же сумму слать одновременно не нужно (сумма будет проигнорирована). `discount_amount` документа — отдельная скидка на весь документ и вычитается из суммы строк. Подробнее — раздел «Скидки» в `FRONTEND_API.md`.
- Ошибки:
