...
> ### ⚠️ Два обязательных пункта (иначе видны текущие баги)
> 1. **Обрабатывайте событие `message_status`** и обновляйте статус по `data.id`.
>    Отправка теперь асинхронная: ответ приходит со статусом `pending`, а `sent`/
...
>    (это и есть жалоба «pending висит на сокете»). Статус в БД при этом уже
>    правильный — переоткрытие чата (REST) покажет верный статус.
...
- Для чат-экрана используйте `ws/wazzup/`. Вариант `ws/wazzup/chat/<chat_id>/`
  (номер **только из цифр**) тоже работает, но **фильтрации по чату на сервере нет**:
  соединение в любом случае получает чат-события всей компании — это нужно, чтобы
  обновлялся список диалогов. Маршрутизируйте по `lead_id` / `chat_id` на клиенте.
...
- **Доставка ровно один раз.** Раньше одно сообщение приходило в сокет трижды
  (рассылка шла в три группы, на которые подписан один и тот же клиент) — это давало
  тройной трафик и тройной ре-рендер. Теперь каждое событие приходит одной копией.
  Дедуп по `id` всё равно оставьте: он защищает от пересечения сокета и REST-истории
  при реконнекте.
...
4. **`status`**: `pending → sent → delivered → read`, либо `failed`. Показывайте
   часики на `pending`, галочки/ошибку по остальным. Бэкенд теперь ВСЕГДА доводит
   исходящее до `sent`/`failed` и присылает `message_status` (при ошибке Wazzup —
   `failed`). Вечного `pending` со стороны сервера быть не должно; но фронту всё
   равно полезен предохранитель: если через 20–30 c после `ack` статус не пришёл —
   дозагрузить историю по REST (§5.2) и показать «Доставка не подтверждена».
5. **Порядок — сортируйте по `timestamp`, не по порядку прихода.** Каждое
   сообщение несёт `timestamp` (== `created_at`): у входящих это время сообщения
   от Wazzup (`dateTime`), у исходящих — время создания в CRM. Сообщения могут
   приходить по сокету не строго по порядку (обрабатываются параллельно), поэтому
   рендерите список, **отсортированный по `timestamp`** (ISO 8601, лексикограф
   сортировки достаточно).
6. **Медиа**: если `text` пустой, а есть `content_uri` — рендерите вложение по
   `media_type`; сервер также кладёт человекочитаемую метку в `text`
   (`📷 [Фотография]` и т.п.) как фолбэк.
7. **Реконнект**: при обрыве переподключайтесь и **дозагрузите историю по REST**
