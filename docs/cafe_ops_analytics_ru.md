...
- Файл: `apps/cafe/migrations/0008_cafe_ops_analytics_splits.py`
- Команда: `python manage.py migrate cafe`

Зависимости: `cafe.0007_order_debt_payments`, `construction.0002_...` (FK на `CashShift`).
...
- **Аналитика официанта:** для пользователей без ролей `owner/admin` order-based аналитика кафе автоматически сужается до заказов текущего пользователя. Это касается, в частности, `sales summary`, выручки, отказов, долгов, дневного отчёта, отчёта по смене, аналитики по официантам и зарплатного отчёта.
...
| `GET .../analytics/waiter-salary/?date_from=&date_to=` | Оклад × (дни/30) + % от личной выручки за период (нужны профили в `waiter-pay-profiles`); для обычного официанта вернётся только его строка, в ответе есть `waiter_label` и `profile_scope` |
