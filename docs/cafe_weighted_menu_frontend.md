...
- [ ] Форма блюда: `is_sold_by_weight`, `sale_unit`, подпись цены за кг/г/порцию
- [ ] Меню / заказ: определение весового блюда по `is_sold_by_weight` (из меню или снимка строки)
- [ ] Ввод `quantity`: step `0.001` (кг) или `1` (г), без принудительного `parseInt`
- [ ] Отображение суммы: `quantity * unit_price`
- [ ] PATCH заказа: одна строка на `menu_item`, обновление `quantity`
- [ ] История / печать: единицы из `menu_item_sale_unit` / снимка строки
- [ ] Возврат: дробный `quantity`, лимит `refundable_quantity`
- [ ] Кухня: формат «{quantity} {sale_unit}»
