...
class WarehouseComprehensiveTests(TestCase):
    ...
    def test_document_requires_warehouse_from(self):
        ...
        Примечание: продажа владельца (SALE без агента) работает в режиме
        мульти-склада, склад резолвится по строкам — для неё warehouse_from на
        уровне clean() не обязателен. Поэтому проверяем инвариант на WRITE_OFF.
        """
        doc = models.Document(
            doc_type=models.Document.DocType.WRITE_OFF,
            warehouse_from=None,
    ...
    def test_post_document_uses_select_related(self):
        ...
        with self.assertNumQueries(28):  # Разумное количество запросов (нет N+1)
            services.post_document(doc)
    ...
    def test_complex_sale_workflow(self):
        ...
        # В долг (credit): кассовое подтверждение не требуется, документ проводится
        # сразу в POSTED. Кассовая продажа (cash) проходит через CASH_PENDING →
        # approve_cash_request — этот путь покрыт в tests_documents.
        doc = models.Document.objects.create(
            doc_type=models.Document.DocType.SALE,
            warehouse_from=self.wh1,
            counterparty=self.client,
    ...
    def test_owner_can_assign_agent_to_specific_warehouse_via_api(self):
        ...
        # Эндпоинт идемпотентный (get_or_create + обновление), поэтому возвращает 200.
        self.assertEqual(response.status_code, 200, response.data)
        ...
        self.assertEqual(str(response.data["assigned_warehouse"]), str(self.wh1.id))
    ...
    def test_agent_with_assigned_warehouse_sees_only_allowed_warehouse_and_cannot_use_other(self):
        ...
        warehouse_ids = {row["id"] for row in rows}
        ...
        self.assertEqual(str(allowed_cart_response.data["warehouse"]), str(self.wh1.id))
    ...
    def test_owner_can_dispatch_goods_to_agent_without_agent_request(self):
        ...
        self.assertEqual(cart_response.status_code, 201, cart_response.data)
        cart_id = cart_response.data["id"]
        self.assertEqual(str(cart_response.data["agent"]), str(agent.id))
