...
class DocumentsTests(TestCase):
    ...
    def test_post_sale_creates_cash_request_and_approve_posts_money(self):
        ...
        # payment_category задаём явно: иначе автовыбор берёт первую категорию в области,
        # а это уже одна из системных (sale/debt/incassation), создаваемых сигналом.
        doc = models.Document.objects.create(doc_type=models.Document.DocType.SALE, warehouse_from=self.wh, counterparty=None, payment_category=self.paycat)
    ...
    def test_partner_cash_incassation(self):
        ...
        # Системные категории «Инкассация» уже созданы сигналом при создании
        # company/branch и company_b — берём существующие, а не создаём дубль.
        cat_a = models.PaymentCategory.objects.get(
            company=self.company, branch=self.branch, system_code="incassation"
        )
